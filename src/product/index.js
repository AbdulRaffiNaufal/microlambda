...
try {
  ...
  console.log(Items);
  // Urutkan item berdasarkan timestamp (asumsi ada field 'createdAt')
  const sortedItems = Items.sort((a, b) => b.createdAt.S - a.createdAt.S);
  return (sortedItems) ? sortedItems.map((item) => unmarshall(item)) : {};
...
try {
  ...
  // Tambahkan timestamp saat pembuatan
  productRequest.createdAt = Date.now().toString();
